...
import { IsEnum, IsNumber, IsOptional, IsString, ValidateNested, IsNotEmpty, IsDateString, IsDate } from "class-validator"; // Adicionado IsDateString e IsDate
...
export class BufferHistoricoDTO {
    @ApiPropertyOptional({ description: 'ID do histórico de buffer (gerado automaticamente)', example: 1 })
    ...
    @ApiProperty({ type: () => ItemDto, description: 'Dados do item associado ao histórico de buffer' })
    @ValidateNested() // Valida as propriedades de ItemDto
    @Type(() => ItemDto) // Transforma o objeto em instância de ItemDto
    item: ItemDto;
...
export class ResMercadosIntermediarioDoSetorDTO {
    ...
    @IsEnum(CONSULTA, { message: 'O tipo de consulta deve ser um valor válido do enum CONSULTA.' })
    consulta: CONSULTA;
