...
export class ConsultaMercadoService {
    constructor(
        ...
        @Inject(MercadosIntermediarioRepository) private mercadoIntermediarioRepo: MercadosIntermediarioRepository,
    ) { }
    ...
    async consultarMercadosExistentesNoSetor(idsetor: number): Promise<MercadosIntermediario[]> {
        try {
            return await this.mercadoIntermediarioRepo.find({
                where: {
                    setor: {
    ...
    async consultarMercadosDoSetor(idSetor: number, dia: Date): Promise<MercadosIntermediario[]> {
        try {
            ...
            if (error instanceof EntityNotFoundError) {
                throw new NotFoundException(`O setor não foi achado com id ${idSetor} nao foi achado}`)
            }
            throw error;
        }
    }
