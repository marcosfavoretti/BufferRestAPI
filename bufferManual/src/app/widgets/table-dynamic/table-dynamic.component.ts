...
import { tableColumns, TableModel } from '../../@core/models/components/table.model';
...
export class TableDynamicComponent implements OnChanges {
  ...
  onNewCheckEvent(row: any, column: any, event: any): void {
    ...
    this.setNestedValue(row, column.path, event.target.value)
    this.onChecked.emit({
      row: row,
      column: column,
  ...
  getTotalOfColumn(column: tableColumns): number | undefined {
    ...
    return this.data.reduce((total, item) => {
      const [targetItens] = this.getNestedValue(item, column.path);
  ...
  getAllColumns(): string[] {
    const ocultColumns = this.tableModel?.ghostControll?.map(col => col.path);
    const visibleColumns = this.tableModel.columns?.map(col => col.path);
  ...
  checkHighLight(data: any): { [key: string]: string } | undefined {
    ...
    for (const controll of this.tableModel.ghostControll) {
      const value = this.getNestedValue(data, controll.path);
